...
def activate(id, app):
    ...
    if result:
        ...
        if status == "Active":
            ...
            update_query = "UPDATE id SET status = %s Where id = %s"
            try:
...
def searchTableDateAdmin(date):
    search_crit = (datetime.strptime(date,"%m/%d/%Y")).timestamp()
...
def searchTableRangeAdmin(date):
    ...
    start_date = start_date_object.timestamp()
    end_date = end_date_object.timestamp()

    search_query = "SELECT * FROM users where Date BETWEEN %s AND %s"

    try:
        cursor.execute(search_query, (start_date,end_date))
        result = cursor.fetchall()
        return result
